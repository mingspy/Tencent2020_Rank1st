...
class ctrNet(nn.Module):
    ...
    def train(self, train_dataset, dev_dataset=None):
        ...
        scheduler = get_linear_schedule_with_warmup(
            ...
            num_warmup_steps=int(
                len(train_dataloader) * args.num_train_epochs * 0.2),
        ...
        for idx in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                del batch
                model.train()
                loss = model(dense_features, text_features, text_ids,
                             text_masks, text_features_1, text_masks_1, labels)
                if args.n_gpu > 1:
                    loss = loss.mean(
                ...
                #测试验证结果
                if (step +
                        1) % args.eval_steps == 0 and dev_dataset is not None:
                    #输出验证集性别和年龄的概率
                    age_probs, gender_probs = self.infer(dev_dataset)
                    #输出性别和年龄的loss和acc
                    age_results = self.eval(dev_dataset.df['age'].values,
                                            age_probs)
                    gender_results = self.eval(dev_dataset.df['gender'].values,
                                               gender_probs)
                    results = {}
                    ...
                    if results['eval_age_acc'] > best_age_acc:
                        ...
                        logger.info("  " + "*" * 20)
                        logger.info("  Best age acc:%s",
                                    round(best_age_acc, 4))
                        logger.info("  " + "*" * 20)
                        try:
                    ...
                    if results['eval_gender_acc'] > best_gender_acc:
                        ...
                        logger.info("  " + "*" * 20)
                        logger.info("  Best gender acc:%s",
                                    round(best_gender_acc, 4))
                        logger.info("  " + "*" * 20)
                        try:
                            os.system("mkdir -p {}".format(args.output_dir))
                        except:
                        ...
                        output_model_file = os.path.join(
                            args.output_dir,
                            "pytorch_model_{}.bin".format('gender'))
                        torch.save(model_to_save.state_dict(),
                                   output_model_file)
                    logger.info("  best_acc = %s",
                                round(best_age_acc + best_gender_acc, 4))
            ...
            if dev_dataset is not None:
                ...
                if results['eval_age_acc'] > best_age_acc:
                    ...
                    model_to_save = model.module if hasattr(
                        ...
                        'module') else model  # Only save the model it-self
                    output_model_file = os.path.join(
                        args.output_dir, "pytorch_model_{}.bin".format('age'))
    ...
    def infer(self, eval_dataset):
        ...
        for batch in eval_dataloader:
            _, dense_features, text_features, text_ids, text_masks, text_features_1, text_masks_1 = (
                x.to(args.device) for x in batch)
            with torch.no_grad():
                probs_1, probs_2 = model(dense_features, text_features,
                                         text_ids, text_masks, text_features_1,
                                         text_masks_1)
    ...
    def reload(self, label):
        ...
        model = self.model
        args = self.args
        args.load_model_path = os.path.join(
            args.output_dir, "pytorch_model_{}.bin".format(label))
