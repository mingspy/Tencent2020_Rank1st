...
def merge_files():
    ...
    click_df = click_df.append(
        pd.read_csv("data/train_semi_final/click_log.csv"))
    ...
    train_user = train_user.append(
        pd.read_csv("data/train_semi_final/user.csv"))
    train_user = train_user.reset_index(drop=True)
    train_user['age'] = train_user['age'] - 1
    train_user['gender'] = train_user['gender'] - 1
    test_user = pd.read_csv("data/test/click_log.csv").drop_duplicates(
    ...
    #for i in range(2):
    #    click_df['gender_{}'.format(i)] = (click_df['gender'] == i).astype(
    #        np.int16)
