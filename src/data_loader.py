...
class TextDataset(Dataset):
    def __init__(self, args, df):
        ...
        for x in args.text_features:
            self.embeddings_tables.append(
                args.embeddings_tables[x[0]] if x[0] is not None else None)
        self.embeddings_tables_1 = []
        for x in args.text_features_1:
            self.embeddings_tables_1.append(
                args.embeddings_tables_1[x[0]] if x[0] is not None else None)
        self.args = args
        self.df = df
    ...
    def __getitem__(self, i):
        ...
        if len(self.args.text_features) == 0:
            ...
            text_features = np.zeros(
                (self.args.max_len_text, self.args.text_dim))
            text_masks = np.zeros(self.args.max_len_text)
            text_ids = np.zeros(
                (self.args.max_len_text, len(self.args.text_features)),
                dtype=np.int64)
            begin_dim = 0
            for idx, (embed_table, x) in enumerate(
                    zip(self.embeddings_tables, self.text_features[i])):
                end_dim = begin_dim + self.args.text_features[idx][-1]
                for w_idx, word in enumerate(
                        x.split()[:self.args.max_len_text]):
                    text_features[w_idx, begin_dim:end_dim] = embed_table[word]
                    text_masks[w_idx] = 1
                    try:
                        text_ids[w_idx, idx] = self.args.vocab[
                            self.args.text_features[idx][1], word]
                    except:
        ...
        if len(self.args.text_features_1) == 0:
            ...
            text_features_1 = np.zeros(
                (self.args.max_len_text, self.args.text_dim_1))
            text_masks_1 = np.zeros(self.args.max_len_text)
            begin_dim = 0
            for idx, (embed_table, x) in enumerate(
                    zip(self.embeddings_tables_1, self.text_features_1[i])):
                end_dim = begin_dim + self.args.text_features_1[idx][-1]
                if embed_table is not None:
                    for w_idx, word in enumerate(
                            x.split()[:self.args.max_len_text]):
                        text_features_1[w_idx,
                                        begin_dim:end_dim] = embed_table[word]
